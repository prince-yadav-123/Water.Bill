<%@ Page Language="C#" MasterPageFile="~/noidajal.master" AutoEventWireup="true"
    CodeFile="Challan_Entry_SINGLE_UPDATE.aspx.cs" Inherits="MainPage_Challan" Title="NoidaJal::Challan Entry" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    function validate()
     {     
        var paydateString = document.getElementById("<%=txt_paid_date.ClientID %>").value;
        var fromdateString = document.getElementById("<%=txt_bill_from.ClientID %>").value;
        var todateString = document.getElementById("<%=txt_bill_to.ClientID %>").value;
        var regex =/(([0-9])|([0-2][0-9])|([3][0-1]))\-(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\-\d{4}$/; 
        if(document.getElementById("<%=ddl_bank.ClientID %>").value=="0")
        {
        alert("Please Select Bank Name");
        document.getElementById("<%=ddl_bank.ClientID %>").focus();
        return false;
        }        
        if(document.getElementById("<%=txt_challan_no.ClientID %>").value=="")
        {
        alert("Please Enter Challan No.");
        document.getElementById("<%=txt_challan_no.ClientID %>").focus();
        return false;
        }           
        if (document.getElementById("<%=txt_paid_date.ClientID %>").value=="")
        {
        alert("Please Enter Pay Date");
        document.getElementById("<%=txt_paid_date.ClientID %>").focus();
        return false;
        }        
        var paydateParts = paydateString.split(/-/);
        var fromdateParts = fromdateString.split(/-/);
        var todateParts = todateString.split(/-/);        
        var pay=paydateParts[2]+'-'+ReturnMonthName(paydateParts[1])+'-'+paydateParts[0];
        var from=fromdateParts[2]+'-'+ReturnMonthName(fromdateParts[1])+'-'+fromdateParts[0];
        var to=todateParts[2]+'-'+ReturnMonthName(todateParts[1])+'-'+todateParts[0];
        if (regex.test(paydateString)) 
        {           
           if(pay < from || pay > to)
           {
           alert("Please enter valid paid date between current period.");
           document.getElementById("<%=txt_paid_date.ClientID %>").focus(); 
           return false;
           } 
        }           
     ...
     function ReturnMonthName(expression)
     {
     switch(expression) {
                          case 'Jan':
                            return '01';
                                        ...
                                        <table width="100%">
                                            ...
                                            <tr>
                                                <td align="center">
                                                    ...
                                                    <asp:ImageButton ID="btnSave" runat="server" ImageUrl="~/images/save.png" OnClick="btnSave_Click" OnClientClick=" return validate()"
