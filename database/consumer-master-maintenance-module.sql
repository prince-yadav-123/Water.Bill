-- Consumer Master Maintenance module for Water.Bill Authority Portal.
-- Uses existing imported table `consumer_details_master`; no schema changes.

SET @moduleName := 'Consumer Master Maintenance';
...
SELECT 1, NULL, @moduleName, 'CM', '/ConsumerMasterMaintenance', 'Consumers', @moduleName, @moduleId, 58, 1, 1, 0, NOW(6), 0
WHERE NOT EXISTS (
...
SELECT r.`Id`, @moduleName, @moduleId, 1, 1, 0, 1, 1, 1, 1, 0, 0, 0, NOW(6), 0
FROM `approles` r
WHERE r.`Name` = 'Admin'
  AND NOT EXISTS (
